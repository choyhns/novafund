...
<body>
	...
	style="width: 500px; max-width: 90%">
		...
		가입한 이메일을 입력해 주세요. 입력한 이메일로 임시 비밀번호를 발송해 드립니다.</p>
		...
		background-color: #20b2aa;" href="<%=cp %>/login.action">로그인하러 가기</a>
		</div>
	</div>
	...
	<script type="text/javascript">
		...
		$(function(){
			...
			$.ajax({
				...
				type:"post",
				url:"searchPwd.action",
				data:params,
				success:function(res){
					if(res=="fail"){
						$("#idCheck").text("발송실패. 이메일을 확인하고 잠시 후 재시도 해주세요.").
						css({"color":"red","font-size":"11pt"}).show();
					}else if(res=="success"){
						$("#idCheck").text("발송완료.").
